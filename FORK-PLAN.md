...
| **Phase 4（P3，按需）** | metrics（Anthropic 適配已由用戶裁決放棄） | 觸發條件成立時再議 | 擱置 |
| **Phase 2.5（缺陷修復）** | 非迴環校驗誤傷非 serve 子命令 → `ValidateServe()`；`.gitattributes` 統一 LF | login/doctor 無 key 可跑；索引全 LF | ✅ **完成（2026-09-19，commit 1685c5c）** |
